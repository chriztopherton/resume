...
def create_timeline_chart(experience_data):
    ...
    fig = px.timeline(df, x_start="Start", x_end="End", y="Company", 
...
def create_skills_heatmap(skills_data):
    ...
    fig = px.imshow(
        pivot_df,
        title="Skills Heatmap by Category",
...
def create_achievement_metrics(experience_data):
    ...
    metrics = {
        ...
        "Technologies Used": len(set(tech for job in experience_data for tech in job["technologies"])),
...
def get_skill_proficiency_level(skill, experience_data):
    ...
    for job in experience_data:
        if skill in job["technologies"]:
            ...
            if "2023" in job["duration"] or "2024" in job["duration"] or "Present" in job["duration"]:
                recent_usage = True
...
def add_download_resume_button():
    ...
    st.info("📄 PDF version coming soon! For now, you can view the full resume in the Experience section.")
